...
def run_cleaning_stage(config: Mapping[str, Any]) -> pd.DataFrame:
    """Execute the cleaning stage for the provided configuration mapping."""
    dropped_rows_path = config.get("dropped_rows_path")
    path = Path(dropped_rows_path) if dropped_rows_path else None
    return clean_data(dict(config), dropped_rows_path=path)
